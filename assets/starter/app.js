(() => {
  ...
  function renderItinerary() {
    ...
    const days = trip.days.filter((day) => dayFilter === 'all' || day.phaseId === dayFilter).map((day) => {
      ...
      return `<details class="card day-card" ${day.dayNumber === 1 ? 'open' : ''}>
        <summary>
  ...
  function renderTools() {
    ...
    app.innerHTML = `${pageHead('Trip tools', '预算与下单清单', '预算区分估算与已确认成本；清单状态只保存在当前设备。')}
      <div class="grid two"><section><div class="budget-total"><span>当前人均估算</span><strong>${money(totals.low)}–${money(totals.high)}</strong><small>另建议预留 ${Math.round((trip.budget.contingencyRate || 0) * 100)}% 机动金</small></div><div class="card">${budgetRows}</div></section>
      <section class="card"><h2>最终下单前检查</h2><ul class="checklist">${checks}</ul><div class="action-row"><button class="button" id="calendar-button">导出日历</button><button class="button secondary" onclick="window.print()">打印 / PDF</button></div></section></div>`;
