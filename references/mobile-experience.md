...
- Tools can use navigation tiles for independent tasks, with native details or accessible accordions inside each task. Choose by content depth, not a fixed button count. Keep summaries, typography and expand icons consistent; preserve checklist state. Printing must include intended collapsed content without duplicate hidden copies. See [interaction-architecture.md](interaction-architecture.md).
